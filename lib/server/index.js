...
module.exports = function(backend, serverOptions, clientOptions) {
  ...
  if (serverOptions.session) {
    ...
    if (backend.shareClient && backend.shareClient.use){
      emitter = backend.shareClient;
    }
...
function doneInitialization(backend, stream, request){
  // sharedb
  if (backend.backend === backend.shareClient || !backend.shareClient) {
    var emitter = backend.backend || backend
...
function createWebSocketStream(client, logger) {
  ...
  client.on('message', function onMessage(data) {
    ...
    // livedb or sharedb < 0.8
    if (backend.backend || backend.shareClient) {
      try {
