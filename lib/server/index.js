...
var isLivedbRacer = false /// < 0.6
var isOldRacer = false // >= 0.6 < 0.8
var isNewRacer = false // >= 0.8

module.exports = function(backend, serverOptions, clientOptions) {

  if (!backend.backend && !backend.shareClient) isNewRacer = true
  else if (backend.backend === backend.shareClient || !backend.shareClient) isOldRacer = true
  else isLivedbRacer = true
  ...
  var middleware = BrowserChannelServer(serverOptions, function(client, connectRequest) {
    ...
    function startBrowserChannel() {
      ...
      var stream = createBrowserChannelStream(client, backend.logger);
  ...
  wss.on('connection', function (client) {
    ...
    var stream = createWebSocketStream(client, backend.logger);
  ...
  if (serverOptions.session) {
    var emitter = backend.shareClient || backend.backend || backend;
...
function doneInitialization(backend, stream, request){
  ...
  if (isLivedbRacer) {
    var agent = backend.shareClient.listen(stream, request);
    done(agent);
  } else {
    var emitter = backend.backend || backend
    emitter.on('connect', function(data){
      var agent = data.agent;
      done(agent);
    });

    emitter.listen(stream, request);
  }
